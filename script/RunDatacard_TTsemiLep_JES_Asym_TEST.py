...
def RunYear(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,Rebinning,pseudo,OneBinFailRegion,OneBin,IsTestJob=False):
    ...
    if OneBin:
        dsuffix+="_OneBin"        
    datacarddir="datacards"+dsuffix+"/TTsemiLep_JES_Asym_TEST/"+Ana+"/"+suffix+"/"+xname
...
def RunWithCondor(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,pseudo,OneBinFailRegion,OneBin):
    ...
    WORKDIR="WORKDIR/TTsemiLep_JES_Asym_TEST/datacard"+statonly_suffix+scalepdf_precalc_suffix+pseudo_suffix+OneBinFailRegion_suffix+OneBin_suffix+"/"+Ana+"/"+Year+"/"+suffix+"/"+cut+"/"+xname
...
def GetRebinning(year,cut,x,suffix):
    ...
    path=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLep_JES_Asym_TEST/RebinInfo/TTsemiLep_JES_Asym_TEST__"+year+"__"+suffix+".py"
        ...
        for TDecayCh in TDecayChs:
            ...
            for PTBIN in PTBINS:
                for ETABIN in ETABINS:
                    cutname=LeptonCh+TDecayCh+"__"+PTBIN+"__"+ETABIN
                    cutlist.append(cutname)
            cutname=LeptonCh+TDecayCh
            cutlist.append(cutname)


    ##---
    parser = argparse.ArgumentParser(description='TTsemiLep_JES_Asym_TEST.py')
