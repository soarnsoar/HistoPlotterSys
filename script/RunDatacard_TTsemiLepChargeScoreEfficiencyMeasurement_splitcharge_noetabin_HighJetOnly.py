...
def RunYear(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,Rebinning,pseudo,OneBinFailRegion,IsTestJob=False):
    ...
    datacarddir="datacards"+dsuffix+"/TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly/"+Ana+"/"+suffix+"/"+xname
    print("datacarddir=",datacarddir)
    mydc=JHDatacard(Year,name,datacarddir,pseudo)
    mydc.NuisanceSkip=\
        ['bChargeID_Jet_Corr_'+PT+ETA+Year+'Down' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]\
        +['bChargeID_Jet_Corr_'+PT+ETA+Year+'Up' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]\
        +['bChargeID_Jet_UnCorrPlus_'+PT+ETA+Year+'Down' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]\
        +['bChargeID_Jet_UnCorrPlus_'+PT+ETA+Year+'Up' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]\
        +['bChargeID_Jet_UnCorrMinus_'+PT+ETA+Year+'Down' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]\
        +['bChargeID_Jet_UnCorrMinus_'+PT+ETA+Year+'Up' for PT in ["PT30To50","PT50To70","PT70To100","PT100To140","PT140ToInf"] for ETA in ["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"]]
    ...
    procpath=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly/proc.py"
    if IsTestJob:
        procpath=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly/proc_test.py"
...
def RunWithCondor(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,pseudo,OneBinFailRegion):
    ...
    WORKDIR="WORKDIR/TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly/datacard"+statonly_suffix+scalepdf_precalc_suffix+pseudo_suffix+OneBinFailRegion_suffix+"/"+Ana+"/"+Year+"/"+suffix+"/"+cut+"/"+xname
...
def GetRebinning(year,cut,x,suffix):
    #config/ForDC/RunDatacard_TTsemiLepChargeScoreEfficiencyMeasurement/RebinInfo/RunDatacard_TTsemiLepChargeScoreEfficiencyMeasurement__2016postVFP__runSys__ApplyBtagSF__use_beff__.py
    GIT_HistoPlotterSys=os.getenv("GIT_HistoPlotterSys")
    #config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement/RebinInfo/TTsemiLepChargeScoreEfficiencyMeasurement__2018__runSys__use_beff_dasym__JETPUID_L__chi2kincut__bdt2512.5__
    path=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly/RebinInfo/TTsemiLepChargeScoreEfficiencyMeasurement__"+year+"__"+suffix+".py"
...
if __name__ == '__main__':
    ...
    suffix="runSys__use_beff_dasym__JETPUID_L__chi2kincut__bdt2512.5__splitcharge__noetabin__HighJetOnly__"
    ...
    ETABINS={
        ...
        "Has_eL":[""],
        "No_eL":[""],
        "NoSL_jH":["Eta0To0p8","Eta0p8To1p6","Eta1p6To2","Eta2To2p5"],
    ...
    for LeptonCh in LeptonChs:
        for TDecayCh in TDecayChs:
            for PASSFAIL in ProbePassFails:
                for PTBIN in PTBINS:
                    for ETABIN in ETABINS[PASSFAIL]:
                        #Lepton_bJetLeptonicSide_NoSL_jH__PT50To70__Eta1p6To2
                        cutname=LeptonCh+TDecayCh+"_"+PASSFAIL+"__"+PTBIN+"__"+ETABIN
                        if ETABIN=="":
    ...
    ##---
    parser = argparse.ArgumentParser(description='RunDatacard_TTsemiLepChargeScoreEfficiencyMeasurement_splitcharge_noetabin_HighJetOnly.py')
