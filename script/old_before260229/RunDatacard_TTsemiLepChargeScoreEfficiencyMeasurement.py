...
def RunYear(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,Rebinning,pseudo,OneBinFailRegion,IsTestJob=False):
    ...
    datacarddir="datacards"+dsuffix+"/TTsemiLepChargeScoreEfficiencyMeasurement/"+Ana+"/"+suffix+"/"+xname
    ...
    procpath=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement/proc.py"
    if IsTestJob:
        procpath=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement/proc_test.py"
...
def RunWithCondor(Ana,Year,suffix,cut,xname,StatOnly,PreCalcScalePDF,DoSimple,pseudo,OneBinFailRegion):
    ...
    WORKDIR="WORKDIR/TTsemiLepChargeScoreEfficiencyMeasurement/datacard"+statonly_suffix+scalepdf_precalc_suffix+pseudo_suffix+OneBinFailRegion_suffix+"/"+Ana+"/"+Year+"/"+suffix+"/"+cut+"/"+xname
...
def GetRebinning(year,cut,x,suffix):
    ...
    path=GIT_HistoPlotterSys+"/config/ForDC/TTsemiLepChargeScoreEfficiencyMeasurement/RebinInfo/TTsemiLepChargeScoreEfficiencyMeasurement__"+year+"__"+suffix+".py"
...
if __name__ == '__main__':
    ...
    suffix="runSys__use_beff_dasym__JETPUID_L__chi2kincut__bdt2512.5__"
    ...
    ##---
    parser = argparse.ArgumentParser(description='RunDatacard_TTsemiLepChargeScoreEfficiencyMeasurement.py')
