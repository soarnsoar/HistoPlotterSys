...
class PlotterSF(PlotterBase):
    ...
    def DrawObjectPad1(self,rm_xtitle=0):
        ...
        for i,hpdict in enumerate(self.list_hpdict):
            ...
            this_h=hpdict["SF"].GetHist()
            this_grerr=hpdict["SF"].GetErrorGraph()
            
            this_h.SetMarkerColor(this_color)
            this_h.SetMarkerStyle(1)
    ...
    def MakeRatios(self):
        ...
        hpdeno=self.list_hpdict[0]["SF"]
        self.list_hpratio=[]
        for i,hpdict in enumerate(self.list_hpdict):            
            this_hp=hpdict["SF"].Divide(hpdeno)
    ...
    def SetMaximum(self):
        ...
        _h=self.list_hpdict[0]["SF"].GetHist()
        Nbins=_h.GetNbinsX()
