...
import math
class JHDatacard:
    def __init__(self,year,region,directory,IsPseudoExp=1):
        print('[JHDatacard__bplus1p1Pass.py]')
        ##
        self.region=region
        ...
        self.IgnoreSmall=1
        self.ScaleMC=False
        self.bplusPassScale=1.
        self.bplusFailScale=1.

        self.bminusPassScale=1.
        self.bminusFailScale=1.
    ...
    def RunWithSKFlatOutput(self,Year,AnalyzerName,cut,x,procpath,suffix):
        ...
        for proc in self.plotter.myreader.ProcConf:
            ...
            if self.IsPseudoExp:
                if "bplus" in proc:
                    self.sum_bplus+=self.plotter.HistColl[proc].GetHist().Integral()
                if "bminus" in proc:
                    self.sum_bminus+=self.plotter.HistColl[proc].GetHist().Integral()

                if "bplus" in proc and "PASS" in cut:
                    print("[Scale For bplus]",self.bplusPassScale,"//","proc=",proc,"cut=",cut)
                    
                    self.observation+=self.plotter.HistColl[proc].GetHist().Integral()*self.bplusPassScale
                    scaled_shape=self.plotter.HistColl[proc].GetHist().Clone()
                    scaled_shape.Scale(self.bplusPassScale)
                    hdata.Add(scaled_shape)

                elif "bplus" in proc and "FAIL" in cut:
                    print("[Scale For bplus]",self.bplusFailScale,"//","proc=",proc,"cut=",cut)
                    
                    self.observation+=self.plotter.HistColl[proc].GetHist().Integral()*self.bplusFailScale
                    scaled_shape=self.plotter.HistColl[proc].GetHist().Clone()
                    scaled_shape.Scale(self.bplusFailScale)
                    hdata.Add(scaled_shape)
                    
                elif "bminus" in proc and "PASS" in cut:
                    print("[Scale For bminus]",self.bminusPassScale,"//","proc=",proc,"cut=",cut)
                    self.sum_bminus+=self.plotter.HistColl[proc].GetHist().Integral()
                    self.observation+=self.plotter.HistColl[proc].GetHist().Integral()*self.bminusPassScale
                    scaled_shape=self.plotter.HistColl[proc].GetHist().Clone()
                    scaled_shape.Scale(self.bminusPassScale)
                    hdata.Add(scaled_shape)

                elif "bminus" in proc and "FAIL" in cut:
                    print("[Scale For bminus]",self.bminusFailScale,"//","proc=",proc,"cut=",cut)
                    self.sum_bminus+=self.plotter.HistColl[proc].GetHist().Integral()
                    self.observation+=self.plotter.HistColl[proc].GetHist().Integral()*self.bminusFailScale
                    scaled_shape=self.plotter.HistColl[proc].GetHist().Clone()
                    scaled_shape.Scale(self.bminusFailScale)
                    hdata.Add(scaled_shape)                                        
                else:
    ...
    def ExportDatacard(self):
        print('<SaveDC>')
        print(self.directory+"/"+self.region+".txt")
